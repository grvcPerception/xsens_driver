...
def wraptopi(x):
        pi = math.pi
        x = x - math.floor(x/(2*math.pi)) *2 *pi
        if x >= math.pi:
            x = x - 2*math.pi
        return x
...
class XSensDriver(object):
    ...
    def reset_vars(self):
        ...
        self.view_msg = PoseStamped();
        self.ang_msg = Float32()
        self.ang_msg2 = Float32()
        self.ang_msg3 = Float32()
        self.pub_imu = False
    ...
    def spin_once(self):
        ...
        if self.pub_imu:
            ...
            self.view_msg.header = self.h
            if self.imu_pub is None:
                self.imu_pub = rospy.Publisher('imu/data', Imu, queue_size=10)
            if self.view_pub is None:
                self.view_pub = rospy.Publisher('imu/view_data', PoseStamped, queue_size=10)
            if self.ang_pub is None:
                self.ang_pub = rospy.Publisher('imu/correction_angle', Float32, queue_size=1)
            if self.ang_pub2 is None:
                self.ang_pub2 = rospy.Publisher('imu/correction_angle_pitch', Float32, queue_size=1)
            if self.ang_pub3 is None:
                self.ang_pub3 = rospy.Publisher('imu/correction_angle_roll', Float32, queue_size=1)
            self.view_pub.publish(self.view_msg)
            self.ang_pub.publish(self.ang_msg)
            self.ang_pub2.publish(self.ang_msg2)
            self.ang_pub3.publish(self.ang_msg3)
